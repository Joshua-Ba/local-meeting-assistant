// llm_engine.cpp
//
// Thin wrapper around llama.cpp: load a model, build a sampler chain,
// and run text generation against a Qwen-style chat template.
...
LlmEngine::LlmEngine(const Config &config)
    : model_(nullptr), ctx_(nullptr), sampler_(nullptr), vocab_(nullptr) {
    ...
    vocab_ = const_cast<llama_vocab*>(
        llama_model_get_vocab(static_cast<llama_model*>(model_)));

    // Sampler chain: temperature -> top-k -> top-p -> min-p -> penalties -> dist.
    llama_sampler_chain_params sampler_params = llama_sampler_chain_default_params();
    sampler_ = llama_sampler_chain_init(sampler_params);
    auto* chain = static_cast<llama_sampler*>(sampler_);
    ...
    llama_sampler_chain_add(chain, llama_sampler_init_penalties(
        128, 1.0f, 0.0f, config.presence_penalty));
...
std::optional<std::string> LlmEngine::generate(const std::string& input,
                                               int max_tokens,
    ...
    const std::string formatted_prompt =
        "<|im_start|>system\n" + std::string(messages[0].content) +
        "<|im_end|>\n<|im_start|>user\n" + std::string(messages[1].content) +
        "<|im_end|>\n<|im_start|>assistant\n<think>\n\n</think>\n\n";

    // Tokenize the prompt.
    int n_prompt = -llama_tokenize(vocab, formatted_prompt.c_str(),
